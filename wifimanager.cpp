...
void dnsBgTask(void *param) {
    ...
    for (;;) {
        yield();
        if (wifimanager->dnsServerActive)
...
void WIFIMANAGER::startBackgroundTask(String softApName, String softApPass) {
    if (softApName.length())
        this->softApName = softApName;
    if (softApPass.length())
        this->softApPass = softApPass;
    loadFromNVS();
    // setMode(WIFI_STA);
    tryConnect();
        ...
        for (uint8_t i = 0; i < WIFIMANAGER_MAX_APS; i++) {
            ...
            if (preferences.getType(tmpKey) == PT_STR) {
                ...
                if (apName.length() > 0) {
                    ...
                    sprintf(tmpKey, "apAddr%d", i);
                    String apAddr = preferences.getString(tmpKey);
                    sprintf(tmpKey, "apGate%d", i);
                    String apGate = preferences.getString(tmpKey);
                    sprintf(tmpKey, "apMask%d", i);
                    String apMask = preferences.getString(tmpKey);
                    sprintf(tmpKey, "apPdns%d", i);
...
bool WIFIMANAGER::writeToNVS() {
    ...
    for (uint8_t i = 0; i < WIFIMANAGER_MAX_APS; i++) {
        ...
        snprintf(tmpKey, sizeof(tmpKey), "apAddr%d", i);
        preferences.putString(tmpKey, apList[i].apAddr);

        snprintf(tmpKey, sizeof(tmpKey), "apGate%d", i);
        preferences.putString(tmpKey, apList[i].apGate);

        snprintf(tmpKey, sizeof(tmpKey), "apMask%d", i);
        preferences.putString(tmpKey, apList[i].apMask);

        snprintf(tmpKey, sizeof(tmpKey), "apPdns%d", i);
        preferences.putString(tmpKey, apList[i].apPdns);
...
bool WIFIMANAGER::delWifi(uint8_t apId) {
    if (apId < WIFIMANAGER_MAX_APS) {
        ...
        apList[apId].apAddr.clear();
        apList[apId].apGate.clear();
...
void WIFIMANAGER::loop(bool force) {
    if (!force && millis() - lastWifiCheckMillis < intervalWifiCheckMillis)
        return;
...
void WIFIMANAGER::attachWebServer(AsyncWebServer *srv) {
    ...
    apiWebHandlers[apiWebHandlerCount++] = &webServer->on((apiPrefix + "/add").c_str(), HTTP_POST, [&](AsyncWebServerRequest *request) {}, NULL,
                                                          [&](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
                                                              ...
                                                              String apAddr = jsonBuffer["apAddr"].as<String>();
                                                              String apGate = jsonBuffer["apGate"].as<String>();
    ...
    apiWebHandlers[apiWebHandlerCount++] = &webServer->on((apiPrefix + "/connect").c_str(), HTTP_POST, [&](AsyncWebServerRequest *request) {}, NULL,
                                                          [&](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
                                                              ...
                                                              if (!apList[networkId].apName.isEmpty())
                                                                  networkName = apList[networkId].apName;
    ...
    <script>
      ...
      function displayNetworks(networks) {
        ...
        async function connectToNetwork(event) {
            ...
            const apAddr = document.getElementById('apAddr').value;
            const apGate = document.getElementById('apGate').value;
